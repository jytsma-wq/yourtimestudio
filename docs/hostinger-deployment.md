...
The app currently uses SQLite through Prisma. The production database file should live in the repository's `db/custom.db`, referenced by an absolute `file:` URL. Before first launch, confirm that Hostinger can write to the `db` directory.
